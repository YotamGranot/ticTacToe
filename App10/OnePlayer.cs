...
namespace App10
{
    [Activity(Label = "OnePlayer")]
    public class OnePlayer : Activity, IOnClickListener
    {
        bool turn = false;
        ...
        public void OnClick(View v)
        {
            ...
            if (btn.Text != "O" && btn.Text != "X")
            {
                ...
                cor = new int[2] { int.Parse(btn.Text[0].ToString()), int.Parse(btn.Text[1].ToString()) };
                btn.Text = "X";
            ...
            if (turnCount != 9)
            {
                string cordinates = board.calculateNextCell();
                cor = new int[2] { int.Parse(cordinates[0].ToString()), int.Parse(cordinates[1].ToString()) };
                buttons[cor[0], cor[1]].Text = "O";
                buttons[cor[0], cor[1]].SetTextColor(Color.Black);
                board.SetCell(cor, turn);
                turnCount++;
                turn = !turn;
                turnText.Text = "X's turn";
